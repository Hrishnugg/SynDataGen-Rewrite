...
const WaveformParticles = () => {
  ...
  useEffect(() => {
    ...
    renderer.setClearColor(0x000011, 1); // Match scene background for consistency
    ...
    // Add render pass and bloom pass
    const renderPass = new RenderPass(scene, camera);
    composer.addPass(renderPass);
    
    // Configure bloom for subtle glow effect (parameters tuned for visuals)
    ...
    // Define specific colors for smooth interpolation
    const leftColor = new THREE.Color(0xff00ff);  // Magenta for left side
    const centerColor = new THREE.Color(0xf0f0ff); // Slightly blue-tinted white for center (less bright)
    ...
    function createParticleTexture() {
      ...
      // Brighter center with softer falloff for the particle glow
    ...
    const handleMouseMove = (event) => {
      ...
      // Remove the scene offset adjustment that was causing misalignment
      // This allows the mouse interactions to align correctly with the visuals
    ...
    const animate = () => {
      ...
      // Render with post-processing for better visuals
      composer.render();
    ...
    const handleResize = () => {
      ...
      waves.forEach((wave, index) => {
        ...
        // Recalculate scale adjustments for the new viewport dimensions
        const newZoomCorrectionFactor = newIsLandscape ? 0.9 : 0.87;

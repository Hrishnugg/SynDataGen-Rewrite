...
import { ProjectsApiDebugInfo } from '@/types/api-responses';
...
export default function ProjectList() {
  ...
  const [debugInfo, setDebugInfo] = useState<ProjectsApiDebugInfo | null>(null);
  ...
  const fetchProjects = async () => {
    try {
      ...
      if (response.status === 401) {
        ...
        throw new Error(`Authentication error: You need to be signed in to view projects. Please refresh the page or sign in again.`);
      }
  ...
  const handleProjectCreated = (newProject: ProjectModel) => {
    // Ensure the new project has all required fields
    const completeProject: ProjectModel = {
      ...newProject,
      // Provide default values for any required fields that might be missing
      ownerId: newProject.ownerId || '',
      teamMembers: newProject.teamMembers || [],
      storageConfig: newProject.storageConfig || { bucketName: '', region: '' },
      settings: newProject.settings || { dataRetentionDays: 30, maxStorageGB: 50 },
    ...
    return (
      <div className="space-y-4">
        <Alert>
          ...
          <AlertDescription>You haven't created any projects yet. Get started by creating your first project.</AlertDescription>
        </Alert>
        
        {debugInfo && debugInfo.noProjects && (
          <Alert className="border-blue-200 bg-blue-50 dark:bg-blue-900/20 dark:border-blue-800">
            <FiInfo className="h-4 w-4 mr-2 text-blue-600 dark:text-blue-400" />
            <AlertTitle className="text-blue-800 dark:text-blue-400">Welcome to Your Dashboard</AlertTitle>
            <AlertDescription className="text-blue-600 dark:text-blue-300">
              This is where your projects will appear once you create them. Click the button below to create your first project.
            </AlertDescription>
          </Alert>
        )}

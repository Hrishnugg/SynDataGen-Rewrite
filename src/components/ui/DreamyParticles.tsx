...
// Import dynamic for client-only component
...
// The main component implementation that will be dynamically imported
function DreamyParticlesComponent({
  ...
  if (!isReady) {
    return (
      <div className="absolute top-0 left-0 w-full h-full overflow-hidden bg-gradient-to-b from-transparent to-amber-500/5" />
    );
  }
...
// This implementation remains the same but will only run on client
function ParticlesScene({
  ...
  // Dynamic imports for heavy browser libraries
  const [browserModules, setBrowserModules] = useState<{
    GLTFLoader?: any;
    DRACOLoader?: any;
  ...
  // Load browser-specific modules only on client
  useEffect(() => {
    const loadModules = async () => {
      try {
        ...
        console.error("Failed to load browser modules:", err);
        setError("Failed to initialize 3D rendering modules");
      }
    };
  ...
  // Load model - modified to use dynamically loaded modules
  useEffect(() => {
    if (!browserModules.GLTFLoader || !browserModules.DRACOLoader) {
      return; // Wait until modules are loaded
    }
    ...
    // Initialize loaders using dynamically imported modules
    const { GLTFLoader, DRACOLoader } = browserModules;
  ...
  }, [browserModules, modelPath, scene]);
...
function ParticleSystem({
  ...
  useEffect(() => {
    ...
    const sampler = new browserModules.MeshSurfaceSampler(mesh).build();
    ...
    const gpuCompute = new browserModules.GPUComputationRenderer(WIDTH, WIDTH, gl);
  ...
  }, [gl, geometry, primaryColor, secondaryColor, mouseStrength, browserModules]);
  ...
  useFrame(({ mouse, clock, camera }) => {
    ...
    const currentMousePos = camera.position
      .clone()
    ...
    velocityVariable.material.uniforms.uMouseSpeed.value =
      mouseVelocity.current * mouseStrength * 3;

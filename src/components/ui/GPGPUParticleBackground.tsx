...
class GPGPUUtils {
  ...
  setupVelocitiesData() {
    ...
    const velocityTexture = new THREE.DataTexture(data, this.size, this.size, THREE.RGBAFormat, THREE.FloatType);
...
class GPGPUEvents {
  ...
  constructor(mouse: MouseHandler, camera: THREE.Camera, mesh: THREE.Mesh, uniforms: any) {
    this.camera = camera;
    this.mouse = mouse;
...
function GPGPU({ config, mousePos }: { config: ParticleConfig, mousePos: { x: number, y: number } }) {
  const { gl, camera } = useThree();
  const [simulationInitialized, setSimulationInitialized] = useState(false);
  const pointsRef = useRef<THREE.Points<THREE.BufferGeometry, ShaderMaterialWithUniforms>>(null);
  ...
  useEffect(() => {
    if (!gl || !meshRef.current || simulationInitialized || !pointsRef.current) return;
    ...
    const events = new GPGPUEvents(mockMouseHandler, camera, meshRef.current, uniforms);
    
    // Set initial uvs and positions from gpgpuUtils
    const geometry = pointsRef.current.geometry;
    geometry.setAttribute('position', new THREE.BufferAttribute(gpgpuUtils.getPositions(), 3));
    geometry.setAttribute('uv', new THREE.BufferAttribute(gpgpuUtils.getUVs(), 2));
  ...
  useFrame(() => {
    ...
    const { gpuCompute, positionVariable, velocityVariable, events } = pointsRef.current.userData;
    
    // Update mouse position
    if (positionVariable.material.uniforms.uMouse && mousePos) {
      positionVariable.material.uniforms.uMouse.value.set(mousePos.x * 2, mousePos.y * 2, 0);
      velocityVariable.material.uniforms.uMouse.value.set(mousePos.x * 2, mousePos.y * 2, 0);
    }
    ...
    pointsRef.current.material.uniforms.uVelocityTexture.value = velocityTexture;
  });
...
export default function GPGPUParticleBackground({ config = {} }: { config?: ParticleConfig }) {

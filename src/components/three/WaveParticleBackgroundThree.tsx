...
function ParticleWave({ mousePos }: {mousePos: {x: number;y: number;};}) {
  // The existing ParticleWave implementation from WaveParticleBackground.tsx
  const pointsRef = useRef<THREE.Points>(null);
  const { size, viewport, clock } = useThree();
  const aspect = size.width / size.height;

  // Particle system properties
  const count = 15000; // High density for fluid-like appearance
  ...
  const MOUSE_FORCE = 0.5; // Strength of mouse interaction
  const MOUSE_RADIUS = 1.5; // Radius of mouse influence
  ...
  const WAVE_INFLUENCE = 0.6; // How much the wave pattern influences particle movement
  const FLUID_INFLUENCE = 0.4; // How much fluid dynamics influence particle movement
  ...
  useFrame(() => {
    ...
    const mouseActive =
    Math.abs(mousePos.x) > 0.01 || Math.abs(mousePos.y) > 0.01;
...
function WaveParticleCanvas({ mousePos }: { mousePos: { x: number; y: number } }) {
  ...
  return (
    <Canvas
      ...
      style={{
        ...
        height: "100%",
        pointerEvents: "none",
        background: theme === "dark" ? "#060B10" : "#060B10",
      }}
    >
      <ParticleWave mousePos={mousePos} />
    </Canvas>
  );
}

// Main export component with explicit client-side rendering safety
export default function WaveParticleBackgroundThree({ mousePos }: { mousePos: { x: number; y: number } }) {
  ...
  return (
    <div className="absolute inset-0 w-full h-full -z-0">
      <WaveParticleCanvas mousePos={mousePos} />
    </div>
  );
} 
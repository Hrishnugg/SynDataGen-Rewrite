...
import { CreateUserInput, USER_COLLECTION } from '@/lib/models/firestore/user';
...
// Create our own minimal type definition to avoid dependency on firebase/auth
...
// Extend NextAuth types to include company field
    ...
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        try {
          ...
          // Pass an empty object as credentials or retrieve them from environment if available
          ...
          try {
            ...
            if (typeof firestoreService.queryDocuments !== 'function') {
              ...
              users = await firestoreService.queryDocuments<UserAuthRecord>(
                USER_COLLECTION,
                [{ field: 'email', operator: '==', value: credentials.email.toLowerCase() }]
              );
            }
            ...
            console.error('[NextAuth:DIAGNOSTIC] Error code:', (queryError as Error)?.name);
            console.error('[NextAuth:DIAGNOSTIC] Error stack:', (queryError as Error)?.stack);
          ...
          // Interface for authentication purposes that includes password
          interface UserAuthRecord extends Record<string, unknown> {
            id: string;
            email: string;
            name: string;
            password: string;
          ...
          const { password, ...userWithoutPassword } = user as UserAuthRecord;
          
          // Make sure we have a valid ID
          // Using type-safe property access
          const userId = user.id || 
            // Check for alternative ID fields that may exist in different DB formats
            (typeof (user as any)._id === 'string' ? (user as any)._id : undefined) || 
            (typeof (user as any).__id === 'string' ? (user as any).__id : undefined);

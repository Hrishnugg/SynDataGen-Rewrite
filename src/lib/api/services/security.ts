...
export class SecurityService {
  ...
  public validateJobConfiguration(jobConfig: JobConfiguration): ValidationResult {
    ...
    if (jobConfig.outputFormat && !supportedFormats.includes(jobConfig.outputFormat)) {
      errors.push(`Output format must be one of: ${supportedFormats.join(', ')}`);
    }
    
    // Validate schema (must have at least one field)
    if (!jobConfig.schema || !jobConfig.schema.fields || jobConfig.schema.fields.length === 0) {
      errors.push('Schema must have at least one field');
    } else {
      // Validate field names (no special characters, no duplicates)
      const fieldNames = jobConfig.schema.fields.map(field => field.name);
      ...
      if (uniqueFieldNames.size !== fieldNames.length) {
        errors.push('Schema contains duplicate field names');
      }
      ...
      for (const field of jobConfig.schema.fields) {
  ...
  public sanitizeJobConfiguration(jobConfig: JobConfiguration): JobConfiguration {
    ...
    if (sanitized.schema && sanitized.schema.fields) {
      sanitized.schema.fields = sanitized.schema.fields.map(field => ({
        ...field,

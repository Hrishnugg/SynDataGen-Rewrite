...
export default function ParticleTest() {
  ...
  const [primaryColor, setPrimaryColor] = useState<[number, number, number]>([1.0, 0.8, 0.3]);
                  ...
                  <div className="absolute bottom-4 left-4 right-4 bg-black bg-opacity-70 rounded-lg p-4">
                    <div className="mb-4">
                      ...
                      >
                        ...
                        <option value="/models/veneciaMask.glb">Venecia Mask</option>
                      </select>
                    </div>
                    
                    <div className="mb-4">
                      <label className="block text-sm font-medium mb-2">Mouse Strength</label>
                      <input 
                        type="range" 
                        min="0" 
                        max="1" 
                      ...
                      {colorOptions.map((color, index) => (
                        <button
                          ...
                          className={`p-2 rounded-lg text-center ${
                            JSON.stringify(primaryColor) === JSON.stringify(color.primary)
                              ? 'ring-2 ring-white'
                          ...
                          onClick={() => {
                            setPrimaryColor(color.primary as [number, number, number]);
                          }}
                        >
                          {color.name}
                        </button>
